...
const Course = () => {
    ...
    useEffect(() => {
        const getCreatorCourse = async () => {
            try {
                const res = await axios.get('https://lms-yuq1.onrender.com/api/v1/course/', { withCredentials: true })
                if (res.data.success) {
                    ...
                    {course?.map((course) => (
                        <TableRow key={course._id}>
                            <TableCell className="md:w-[300px] flex items-center gap-2 ">
                                <img src={`https://lms-yuq1.onrender.com${course.courseThumbnail}`} alt="Thumbnail" className='w-20 hidden md:block rounded-sm'/>
                                {course.courseTitle}
                            </TableCell>
